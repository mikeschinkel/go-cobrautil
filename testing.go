...
func RunCLICommandTest(c4t testutil.ContextForTests, successFunc OnCLITestSuccessFunc, test CLICommandTest) (result any, err error) {
	...
	result = cmdResult
	if runTestSuccessChecksFails(c4t, test) {
		goto end
	}
...
func runTestSuccessChecksFails(c4t testutil.ContextForTests, test CLICommandTest) (failed bool) {
	c4t.Helper()
